...
@MainActor
class MemberDataViewModel : ObservableObject{
    ...
    @Published var savedUserDaten : [MemberEntity] = []
    ...
    func updateMemberData(MemberData: MemberEntity){
    ...
    func addNewMember(_ desc : String){
        let newUser = MemberEntity(context: container.viewContext)
        newUser.desc = desc
        newUser.done = false
        saveData()
    }
